    ...
    public class ManualQueueControl : IDisposable
    {
        ...
        private void work()
        {
            while (true)
            {
                ...
                ManualSyncRequest syncItem = null;
                lock (locker)
                {
                    if (jobs.Count > 0)
                    {
                ...
                if (item != null)
                {
                    if ((syncItem = item as ManualSyncRequest) != null)
                        ManualRequestHelper.Sync(syncItem);
                    else
                        ManualRequestHelper.Compare(item as ManualCompareRequest);
                }
                else
                {
                    wh.WaitOne();
